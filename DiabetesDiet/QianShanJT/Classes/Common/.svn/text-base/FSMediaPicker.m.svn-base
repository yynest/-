...
- (void)delegatePerformFinishWithMediaInfo:(NSDictionary *)mediaInfo
{
    if ([[mediaInfo allKeys] containsObject:UIImagePickerControllerEditedImage]) {
        ...
        dic[UIImagePickerControllerCircularEditedImage] = [image circularImage];
...
- (void)takePhotoFromCamera
{
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(imagePickerControllerDidCapture2) name:@"_UIImagePickerControllerUserDidRejectItem" object:nil];
        ...
        if (_maskImageView) {
            [imagePicker.view addSubview:_maskImageView];
        }
...
- (void)imagePickerControllerDidCapture
{
    NSLog(@"hello world");
    _maskImageView.hidden = YES;
}

- (void)imagePickerControllerDidCapture2
{
    NSLog(@"hello earth");
