...
//    [self requestGetUserQrcode];
    ...
    //意见反馈
//    [self requestSendUserAdvice];
    
    //查询关联关系
//    [self requestGetShareList];
    
    //删除关联关系
//    [self requestDealShareDelete];
}
...
/user/advice 意见反馈
 参数:
 picUrls (Array[string], optional): 图片地址 ,
 suggest (string, optional): 建议
 返回值：
 errCode (integer, optional): 错误代码 ,
 errMsg (string, optional): 错误信息
 data (object, optional),
 */
- (void)requestSendUserAdvice {
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] init];
    NSArray *imgUrls = @[@"http://123",@"http://1234"];
    [dic sf_setObject:imgUrls forKey:@"picUrls"];
    [dic sf_setObject:@"建议" forKey:@"suggest"];
...
/*
 接口:
 /bsrecord/share/delete 删除关联关系
 参数:
 bsRecordShareId (integer, optional)
 返回值：
 errCode (integer, optional): 错误代码 ,
 errMsg (string, optional): 错误信息
 data (object, optional),
 */
- (void)requestDealShareDelete {
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] init];
    [dic sf_setObject:[NSNumber numberWithInteger:254] forKey:@"bsRecordShareId"];
