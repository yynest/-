...
-(void)drawRect:(CGRect)rect{
    ...
    for (int i = 0; i <= _betweenNumber; i ++){
        ...
        //4.0    5.0   6.0
            NSString *num = [NSString stringWithFormat:@"%.1f",i*_step+_minValue];
            if ([num floatValue]>1000000){
                num = [NSString stringWithFormat:@"%.f万%@",[num floatValue]/10000.f,_unit];
            }
            
         //   NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:[UIColor colorTextGragLight]};
            NSDictionary *attribute2 = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:[UIColor colorRulerBorderGarg]};
            CGFloat width = [num boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:0 attributes:attribute2 context:nil].size.width;
            [num drawInRect:CGRectMake(startX+lineCenterX*i-width/2, longLineY+15, width, 14) withAttributes:attribute2];
            CGContextAddLineToPoint(context, startX+lineCenterX*i, longLineY + 10);
        }else{
            CGContextAddLineToPoint(context, startX+lineCenterX*i, shortLineY + 10 );
        }
...
-(void)drawRect:(CGRect)rect{
    ...
    if ([num floatValue]>1000000) {
        num = [NSString stringWithFormat:@"%.1f万%@",[num floatValue]/10000.f,_unit];
    }
    NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:[UIColor colorWithWhite:TextColorGrayAlpha alpha:1]};
    CGFloat width = [num boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:0 attributes:attribute context:nil].size.width;
...
-(instancetype)initWithFrame:(CGRect)frame theMinValue:(float)minValue theMaxValue:(float)maxValue theStep:(float)step theUnit:(NSString *)unit theNum:(NSInteger)betweenNum{
    ...
    if (self) {
        ...
        _bgColor    = [UIColor colorBGGragLighter];
...
-(DYTriangleView *)triangle{
    if (!_triangle) {
        _triangle = [[DYTriangleView alloc]initWithFrame:CGRectMake(self.bounds.size.width/2-0.5-TrangleWidth/2, CGRectGetMaxY(_valueTF.frame) + 5 , TrangleWidth, TrangleWidth)];
...
-(UITextField *)valueTF{
    if (!_valueTF) {
        _valueTF                          = [[UITextField alloc]initWithFrame:CGRectMake(self.bounds.size.width/2-60, 0,80, 40)];
        _valueTF.userInteractionEnabled   = YES;
        _valueTF.defaultTextAttributes    = @{
                                              NSFontAttributeName:[UIFont systemFontOfSize:30],
                                              NSForegroundColorAttributeName:[UIColor colorMainGreen]};
        ...
        UILabel *mmolLabel = [[UILabel alloc]initWithFrame:CGRectMake(_valueTF.right + 10, 10, 80, 20)];
        mmolLabel.text = @"mmol/L";
        mmolLabel.textColor = [UIColor colorTextGrag];
        [self addSubview:mmolLabel];
    }
    return _valueTF;
}
-(UILabel *)unitLab{
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (_scrollByHand) {
        if (totalValue >= 33.3) {
            _valueTF.text = [NSString stringWithFormat:@"33.3"];
          
            [UIView animateWithDuration:1.0 animations:^{
                scrollView.contentOffset = CGPointMake(((33.3-_minValue)/(float)_step)*RulerGap, 0);
            }];
