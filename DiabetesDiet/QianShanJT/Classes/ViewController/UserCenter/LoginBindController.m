...
@interface LoginBindController ()<UITextFieldDelegate>
...
@implementation LoginBindController{
...
- (void)prepareUi{
    ...
    logoView.layer.cornerRadius = 32.0;
    logoView.clipsToBounds = YES;
    NSURL * headUrl = [NSURL URLWithString:[[NSUserDefaults standardUserDefaults] objectForKey:WeiChatImageUrl]];
    [logoView sd_setImageWithURL:headUrl placeholderImage:[UIImage imageNamed:@"defaultHead"]];
    logoView.frame = CGRectMake(viewWidth/2 - 32, 60 + offsetY, 64, 64);
    [self.view addSubview:logoView];
    
    UILabel *bindLabel = [[UILabel alloc] init];
    bindLabel.frame = CGRectMake(0, CGRectGetMaxY(logoView.frame) + 20, viewWidth, 30);
    bindLabel.text = @"为了提供更好的服务，请绑定手机号";
    bindLabel.textColor = [UIColor colorTextBlack];
    ...
    _mobileField.frame = CGRectMake(30, CGRectGetMaxY(bindLabel.frame) + 20, viewWidth - 60, 50);
    ...
    _codeBtn.frame = CGRectMake(CGRectGetMaxX(_mobileField.frame) - 109, CGRectGetMaxY(bindLabel.frame) + 20, 109, CGRectGetHeight(_mobileField.frame));
    ...
    [_loginBtn addTarget:self action:@selector(requestUserBind) forControlEvents:UIControlEventTouchUpInside];
    [self setLoginBtnStatus:NO];
    [self.view addSubview:_loginBtn];
    
    //返回按钮
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    backBtn.frame = CGRectMake(0, 20, 67, 44);
    [backBtn setImage:[UIImage imageNamed:@"leftArrow"] forState:UIControlStateNormal];
    [backBtn setImage:[UIImage imageNamed:@"leftArrowHigh"] forState:UIControlStateHighlighted];
...
- (void)handleUploadWechatImage{
    ...
    [tempImageView sd_setImageWithURL:headUrl placeholderImage:[UIImage imageNamed:@"defaultHead"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        
        [[QNManager sharedInstance] putDataType:@"1" data:UIImagePNGRepresentation(image) suffix:@"jpg" complete:^(BOOL isSuccess, NSString * _Nullable path) {
            [self requestUpdateUserInfo:path];
        }];
    }];
  
}
- (void)requestUpdateUserInfo:(NSString *)path {
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] init];
    [dic sf_setObject:path forKey:@"headPic"];
...
- (void)requestApiWithSendVerifyCode {
    ...
    [dic sf_setObject:[NSNumber numberWithInt:2] forKey:@"source"];
...
- (void)requestUserBind {
    ...
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] init];
    [dic sf_setObject:_mobileField.text forKey:@"identity"];
    [dic sf_setObject:_mobileField.text forKey:@"mobile"];
    [dic sf_setObject:[NSNumber numberWithInt:2] forKey:@"platform"];
    [dic sf_setObject:[[NSUserDefaults standardUserDefaults] objectForKey:WeiChatUnionId]  forKey:@"unionid"];
    ...
    [api startWithCompletionBlockWithSuccess:^(QSBaseRequest *request) {
        ...
        if (error == 0) {
            ...
            NSThread*thread = [[NSThread alloc] initWithTarget:self selector:@selector(handleUploadWechatImage) object:nil];
            [thread start];
