...
-(void)loadPhote:(NSInteger)index{
    ...
    id currentPhotoView = [_subViewArray sf_objectAtIndex:index];
    if (![currentPhotoView isKindOfClass:[PhotoView class]]) {
        ...
        if ([[self.imagesArray firstObject] isKindOfClass:[UIImage class]]) {
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoImage:[self.imagesArray sf_objectAtIndex:index]];
            ...
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoUrl:[self.imagesArray sf_objectAtIndex:index]];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewArray replaceObjectAtIndex:index withObject:photoV];
            self.photoView=photoV;
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    for (UIView *view in scrollView.subviews) {
        if ([view isKindOfClass:[PhotoView class]]) {
            PhotoView *photoV=(PhotoView *)[_subViewArray sf_objectAtIndex:page];
...
-(UIPageControl *)pageControl{
    if (_pageControl==nil) {
        ...
        _pageControl.currentPageIndicatorTintColor = [UIColor colorMainGreen];
        _pageControl.pageIndicatorTintColor = [UIColor colorTextGrag];
