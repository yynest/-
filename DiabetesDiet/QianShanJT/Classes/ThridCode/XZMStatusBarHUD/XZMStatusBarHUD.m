...
- (void)showWindow
{
    ...
    //添加阴影
    window_.layer.shadowColor = [UIColor blackColor].CGColor;
    window_.layer.shadowOpacity = 1.0;//阴影透明度，默认0
    window_.layer.shadowRadius = 4;//阴影半径，默认3
...
- (void)showMessage:(NSString *)message image:(UIImage *)image position:(CGFloat)position animaDelay:(CGFloat)animaDelay configuration:(void (^)())configurationBlock
{
    ...
    btn.imageEdgeInsets = UIEdgeInsetsMake(10, -120, 0, 0);
    btn.titleEdgeInsets = UIEdgeInsetsMake(10, -120+20, 0, 0);
    ...
    imageView.frame = CGRectMake(20, (CGRectGetHeight(self.statusbackgroundView.frame)-25)/2+10, 25, 25);
    [self.statusbackgroundView addSubview:imageView];
    
    
    UILabel *msgLabel = [[UILabel alloc] init];
    msgLabel.font = self.attributedText[NSFontAttributeName];
    msgLabel.textAlignment = NSTextAlignmentLeft;
    msgLabel.textColor = self.attributedText[NSForegroundColorAttributeName];
    msgLabel.numberOfLines = 0;
    msgLabel.frame = CGRectMake(CGRectGetMaxX(imageView.frame) + 15, CGRectGetMinY(imageView.frame), MAIN_SCREEN_WIDTH - CGRectGetMaxX(imageView.frame) - 40, 25);
    msgLabel.text = message;
    [self.statusbackgroundView addSubview:msgLabel];
    ...
    /** 创建定时器 */
    timer_ = [NSTimer scheduledTimerWithTimeInterval:3.0 target:self selector:@selector(hidden) userInfo:nil repeats:YES];
}
...
{
    [self showMessage:error image:[UIImage imageNamed:@"loginCodeError"] position:position animaDelay:animaDelay configuration:configurationBlock];
}
...
- (UIColor *)statusColor
{
    if (_statusColor == nil) {
        ...
        _statusColor =  RGBACOLOR_HEX(0xff5a5a, 1.0);
    }
   return _statusColor;
}
