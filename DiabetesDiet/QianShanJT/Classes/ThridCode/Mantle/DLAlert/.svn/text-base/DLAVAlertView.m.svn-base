...
- (id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitle, ...{
	...
	if (self) {
		...
		_minContentWidth = [UIScreen mainScreen].bounds.size.width - 70;
		_maxContentWidth = [UIScreen mainScreen].bounds.size.width - 70;
...
+ (void)initialize  {
	if ([self class] == [DLAVAlertView class]) {
        //修改整个页面背景色
		[[DLAVAlertViewController sharedController] setBackdropColor:[UIColor colorWithWhite:0.0 alpha:0.5]];
	}
}
...
- (void)addTextFieldWithText:(NSString *)text placeholder:(NSString *)placeholder {
	UITextField * tf = [self internalAddTextFieldWithText:text placeholder:placeholder];
    [tf addTarget:self action:@selector(textFieldEditChanged:) forControlEvents:UIControlEventEditingChanged];
}
//xiangfeng
- (void)textFieldEditChanged:(UITextField *)textField
{
    float lenth = textField.text.length;
    if (lenth>_textFieldMaxLength) {
        NSMutableString *tempStr = [[NSMutableString alloc] initWithString: textField.text];
        [tempStr deleteCharactersInRange:NSMakeRange(_textFieldMaxLength, tempStr.length-_textFieldMaxLength)];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    //xiangfeng
    float existedLength = [ToolBox getSizeLengthWithString:textField.text];
    NSInteger selectedLength = range.length;
    float replaceLength = [ToolBox getSizeLengthWithString:string];
    if (_textFieldMaxLength == 0) {
        _textFieldMaxLength = 50;
    }
    if (existedLength - selectedLength + replaceLength > _textFieldMaxLength) {
        return NO;
    }
    return YES;
}
